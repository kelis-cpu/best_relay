...
class message {
  public:
    ...
    message(int _root, int _src, int _dst, int _step, double _send_time, double _recv_time, int _hop = 1) : 
        root(_root), src(_src), dst(_dst), step(_step), send_time(_send_time), recv_time(_recv_time), hop(_hop) {
...
//
  ...
  public:
    ...
    virtual void send_dupinv(message&) {}
    virtual void receive_getdata(message&) {}
...
static graph lrtpa_G(n);
static vector<std::map<int, priority>> priority_table(N); // k:node_id, v: priority

static double theta0 = 3.0;
static double theta1 = -1.5;
static double min_avg_latency = 1e9;

const static double PRIORITY = 10.0;
const static double MAX_PRIORITY = 255.0;
static double duptxn = 2.0;
static double getdatatxn  = 1.0;

template <int fanout = FANOUT>
class lrtpa : public basic_algo {
    public:

        static void init(int n, LatLonCoordinate *coord) {
            ...
            for (int u = 0; u < n; u++) {
                ...
                for (int k = 0; k < dg; k++) {
                    int v = random_num(n);
                    while (lrtpa_G.add_edge(u, v) == false)
                        v = random_num(n);
                    priority_table[u][v].priority = PRIORITY;
                }
            }
        }
        lrtpa(int n, LatLonCoordinate *coord, int root = 0) {}
        static const char* get_algo_name() {return "LRTPA";}

        vector<int> respond(message msg) {
            int hop = msg.hop + 1;
            int dst = msg.dst;
            vector<int> nb_u = lrtpa_G.outbound(dst);
            vector<int> ret;
            
            int relay_cnt = relay_cnts(hop);
            int tmp_cnt = relay_cnt;
            ...
            if (relay_cnt <= FANOUT) {
                ...
                auto& pri_table = priority_table[msg.dst];
                sort(ret.begin(), ret.end(), [&pri_table](int i, int j) {
                    return pri_table[i].priority > pri_table[j].priority;
                });
                return vector<int>(ret.begin(), ret.begin() + min(added_cnt, relay_cnt));
            }
        ...
        void receive_getdata(message& msg) {
            int src = msg.src;
            int dst = msg.dst;
            priority_table[src][dst].getdatatxn += 1;
            if (priority_table[src][dst].getdatatxn >= 2) {
                priority_table[src][dst].priority += calculate_priority(priority_table[src][dst]);
                priority_table[src][dst].getdatatxn = 0;
                priority_table[src][dst].duptxn = 0;
            }
        }

        void send_dupinv(message& msg) {
            int src = msg.src;
            int dst = msg.dst;
            priority_table[src][dst].duptxn += 1;
            if (priority_table[src][dst].duptxn >= 2) {
                priority_table[src][dst].priority += calculate_priority(priority_table[src][dst]);
                priority_table[src][dst].getdatatxn = 0;
                priority_table[src][dst].duptxn = 0;
            }
        ...
        static const bool specified_root = false;

    private:
    
        double logisticFunction(double hop) {
            // 计算指数部分
            double exponent = theta0 + theta1 * hop;
            // 计算 e 的指数幂
            double expResult = std::exp(exponent);
            // 计算最终结果
            return 1.0 / (1.0 + expResult);
        }
        int relay_cnts(int hop) {
            if (hop <= 2) {
                return 128;
            }
            return 8 * logisticFunction(hop);
        }

        double calculate_priority(priority& pri) {
            
          return pri.blocktxn * 1.0 - pri.duptxn * duptxn + pri.getdatatxn * getdatatxn;
        }
...
class test_result {
  public :
    ...
    test_result() : avg_bnd(0), avg_latency(0), avg_inv_cnts(0), latency(21, 0), 
...
test_result single_root_simulation(int root, int rept_time, double mal_node, shared_ptr<algo_T> algo, int algo_type) {
    ...
    for (int rept = 0; rept < rept_time; rept++)  {
        ...
        for (; !msg_queue.empty(); ) {
            ...
            if (recv_flag[u] == true) {
                dup_msg++;
                if (algo_type == PTPNS) {
                    algo -> send_dupinv(msg);
                }
                continue;
            }
            //msg.print_info();
            if (algo_type == PTPNS) {
                algo -> receive_getdata(msg);
            }
            recv_flag[u] = true;
            recv_time[u] = msg.recv_time;
            ...
            for (auto v: relay_list) {
                ...
                message new_msg = message(root, u, v, msg.step + 1, recv_time[u] + delay_time, recv_time[u] + dist + delay_time, msg.hop + 1);
                msg_queue.push(new_msg);
            }
        }
        ...
        int cnt = 0;
        double pct = 0.05;
        for (pct = 0.05; pct <= 1; pct += 0.05, cnt++) {
            int i = non_mal_node * pct;
            result.latency[cnt] += recv_time[recv_list[i]];
        }
        int sz = recv_list.size();
        result.latency[cnt] += recv_time[recv_list[sz - 1]];
...
test_result simulation(int algo_type, int rept_time = 1, double mal_node = 0.0, bool test_mode = false) {
...
// Then, for every honest node, do a single_root_simulation.
    ...
    if (algo_type == LRTPA || algo_type == PTPNS) {
        lrtpa<>::init(n, coord);
    }
    ...
    for (int rept = 0; rept < rept_time; rept++) {
        ...
        for (; test_node > 0; test_node--) {
            ...
            test_time++;
            auto res = single_root_simulation<algo_T>(root, 1, mal_node, algo, algo_type);
            //printf("%d\n", test_node);
            result.avg_bnd += res.avg_bnd;
            result.avg_inv_cnts += res.avg_inv_cnts;
            for (size_t i = 0; i < result.latency.size(); i++) {
    ...
    if (test_mode) {
        min_avg_latency = std::min(min_avg_latency, result.avg_latency);
        printf("theta0: %f, theta1: %f, duptxn: %f, getdatatxn: %f, avg_latency: %f, avg_band: %f\n", theta0, theta1, duptxn, getdatatxn, result.avg_latency, result.avg_bnd);
        return result;
    }
    ...
    printf("#node, mal node, Bandwidth, dupinv, ");
    for (double p = 0.05; p <= 1.05; p += 0.05) {
        fprintf(output, "%.2f, ", p);
        printf("%.2f, ", p);
    }
    fprintf(output, "\n");
    ...
    fprintf(output, "%d, %d, %.2f, %.2f", n, result.avg_inv_cnts, mal_node, result.avg_bnd);
    std::cout << "total: " << result.avg_inv_cnts << std::endl;
    printf("%d, %d, %.2f, %.2f", n, result.avg_inv_cnts, mal_node, result.avg_bnd);
    //printf
    int cnt = 0;
    for (double p = 0.05; p <= 1.05; p += 0.05, cnt++) {
...
void test_params(int rept, double mal_node) {
    for (double t0 = 3.0; t0 <= 5.0; t0 += 0.1) {
        for (double t1 = -1.5; t1 >= -2.0; t1 -= 0.1) {
            theta0 = t0;
            theta1 = t1;
            simulation<lrtpa<8>>(LRTPA, rept, mal_node, true);
        }
    }
}

void test_priority(int rept, double mal_node) {
    for (double dup = 1.0; dup <= 10.0; dup += 1.0) {
        for (double get = 1.0; get <= 10.0; get += 1.0) {
            duptxn = dup;
            getdatatxn = get;
            simulation<lrtpa<8>>(PTPNS, rept, mal_node, true);
        }
    }
}
...
int main() {
    ...
    // simulation<k_means_cluster<128, 8, 8, true> >(LRTPA, rept, mal_node);
    //MERCURYCLUSTER(with the early outburset optimization disabled)
    //simulation<k_means_cluster<8, 8, 8, true> >(rept, mal_node);

    //RANDOM
    // simulation<random_flood<8, 8, 8> >(RANDOM, rept, mal_node);

    //Perigee
    // simulation<perigee_ubc<6, 6, 8> >(PERIGEE, rept, mal_node);
    
    //BlockP2P
    // k_means();
    // simulation<block_p2p<8> >(BLOCKP2P, rept, mal_node);

    // latpa
    
    // theta0 = 3.0;
    // theta1 = -1.5;
    // simulation<lrtpa<8>>(LRTPA, rept, mal_node);
    // test_params(rept, mal_node);

    // PTPNS
    theta0 = 3.7;
    theta1 = -1.9;
    simulation<lrtpa<8>>(PTPNS, rept, mal_node);
    // test_priority(rept, mal_node);
